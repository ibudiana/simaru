...
import { Building, Info, ArrowRight, CheckCircle2 } from "lucide-react";
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-lg text-muted-foreground mt-2">
            Temukan dan reservasi ruang kerja terbaik untuk aktivitas Anda.
          </p>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {rooms.map((room: (typeof rooms)[number]) => (
            <Card
              key={room.id}
              className="group overflow-hidden border-none shadow-sm hover:shadow-xl transition-all duration-300 bg-white rounded-2xl flex flex-col h-full"
            >
              <div className="h-48 bg-zinc-100 flex items-center justify-center relative overflow-hidden">
                ...
                <Badge
                  className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm text-zinc-900 border-none shadow-sm font-bold"
                  variant="outline"
                >
                  {room.capacity} Pax
                </Badge>
              ...
              <CardContent className="flex-1 flex flex-col justify-between pt-0 mt-4">
                <div className="space-y-4">
                  <div className="flex flex-wrap gap-1.5">
                    {room.facilities.map(
                      (rf: (typeof room.facilities)[number]) => (
                        <span
                          key={rf.facilityId}
                          className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-zinc-100 text-zinc-600 border border-zinc-200"
                        >
                          {rf.facility.name}
                        </span>
                      ),
                ...
                <div className="mt-8">
                  <Link href={`/requester/book?roomId=${room.id}`}>
                    <Button className="w-full rounded-xl py-6 font-bold shadow-lg shadow-primary/20 group-hover:-translate-y-0.5 transition-transform">
                      Mulai Reservasi
                    </Button>
                  </Link>
                </div>
          ...
          {rooms.length === 0 && (
            <div className="col-span-full py-20 flex flex-col items-center justify-center bg-zinc-50 rounded-3xl border-2 border-dashed border-zinc-200">
              ...
              <p className="text-zinc-500 font-medium">
                Maaf, saat ini tidak ada ruangan yang dapat ditampilkan.
              </p>
            </div>
          )}
        </div>
