...
import { CheckCircle, X, Check, User as UserIcon, Calendar as CalendarIcon, MapPin, Building } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Persetujuan Akhir</h1>
          <p className="text-muted-foreground mt-2">Daftar reservasi ruangan yang membutuhkan persetujuan Anda sebagai Kaprodi/Pimpinan.</p>
        </div>
      </div>
      ...
      {pendingApprovals.length === 0 ? (
        <Card className="flex flex-col items-center justify-center p-12 text-center text-muted-foreground border-dashed">
          ...
          <p>Belum ada permintaan yang masuk ke tahap akhir. Halaman sudah bersih!</p>
        </Card>
      ) : (
        <div className="grid gap-6">
          {pendingApprovals.map((stage) => {
            ...
            return (
              <Card key={stage.id} className="overflow-hidden shadow-sm border-blue-200">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      ...
                      <div className="flex items-start">
                        ...
                        <div>
                          <span className="font-semibold block text-sm">Pemohon</span>
                      ...
                      <div className="flex items-start">
                        ...
                        <div>
                          <span className="font-semibold block text-sm">Jadwal Penggunaan</span>
                          <span className="text-sm max-w-sm block">
                            {formatDate(new Date(res.schedule.startTime))} | {formatTime(new Date(res.schedule.startTime))} - {formatTime(new Date(res.schedule.endTime))}
                          </span>
                        </div>
                      </div>

                      <div>
                        <span className="font-semibold block text-sm mb-1">Tujuan:</span>
                        <p className="text-sm bg-muted p-2 rounded-md">{res.purpose}</p>
                      </div>
