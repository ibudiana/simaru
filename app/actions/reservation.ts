...
export async function submitReservation(prevState: any, formData: FormData) {
  ...
  if (aggregators.length === 0) {
    return { error: 'Sistem belum memiliki Admin (Aggregator) untuk meninjau.' }
  }
  ...
  await prisma.$transaction([
    ...
    prisma.notification.create({
      data: {
        ...
        message: 'Permintaan reservasi baru membutuhkan tinjauan Anda sebagai Admin.',
  ...
  // --- Send Emails ---
  const requester = await prisma.user.findUnique({ where: { id: session.userId }, select: { email: true, name: true } })
  const admin = await prisma.user.findUnique({ where: { id: assignedAdminId }, select: { email: true, name: true } })
