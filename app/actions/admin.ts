...
export async function processAdminApproval(formData: FormData) {
  ...
  if (action === 'REJECT') {
    ...
    const finalApprover = await prisma.user.findUnique({ where: { id: finalApproverId }, select: { email: true, name: true } })
    ...
    if (finalApprover?.email) {
      ...
      }).catch(err => console.error("Background Email Error (Approver):", err));
    }
...
function calculateDateRange(filter: {
  ...
  switch (filter.type) {
    ...
    case 'yearly':
      ...
      break
    case 'custom':
      start = filter.startDate ? startOfDay(parseISO(filter.startDate)) : startOfMonth(now)
      end = filter.endDate ? endOfDay(parseISO(filter.endDate)) : endOfMonth(now)
      break
    default:
...
export async function getReportData(filter: {
  ...
  return reservations.map(r => ({
    ...
    'Tanggal': new Intl.DateTimeFormat('id-ID', { dateStyle: 'long' }).format(new Date(r.schedule.startTime)),
