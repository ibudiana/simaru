  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        toast.error("Tidak ada data reservasi untuk periode yang dipilih", {
          id: toastId,
        });
        return;
      }
      ...
      if (reportType === "monthly")
        filename += `_${selectedYear}_${parseInt(selectedMonth) + 1}`;
        ...
        >
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            {isStatsLoading ? (
              ...
              <Activity className="h-4 w-4 text-zinc-400 group-hover:text-primary transition-colors" />
            )}
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-black">{stats.total}</div>
            <p className="text-xs text-zinc-400 mt-1 uppercase tracking-tighter font-bold">
              Periode Terpilih
            </p>
          </CardContent>
        </Card>
      ...
      <Card className="border-none shadow-xl bg-white rounded-3xl overflow-hidden">
        <CardHeader className="text-zync-900 px-6 pt-6 pb-10 space-y-4">
          ...
          <CardDescription className="text-zinc-400 text-base leading-relaxed max-w-md">
            Pilih kriteria dan periode laporan yang ingin Anda ekspor ke format
            Spreadsheet.
          </CardDescription>
        </CardHeader>
        <CardContent className="p-8 space-y-8">
          ...
          <div className="pt-4">
            <Button
              ...
              className="w-full bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl py-8 font-bold text-lg shadow-xl shadow-emerald-100 transition-all hover:-translate-y-0.5 active:scale-95"
            >
              {isDownloading ? (
            ...
            <p className="text-center text-zinc-400 text-sm mt-6 flex items-center justify-center gap-2">
              ...
              Data akan diekspor dalam format .csv yang kompatibel dengan Excel
              & Google Sheets.
            </p>
          </div>
        </CardContent>
